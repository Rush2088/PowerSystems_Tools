...
import { snapToEdge } from '../utils/edgeDetect';

const MAG_SIZE    = 192;  // magnifier canvas size in px (~2 in at 96 dpi)
const MAG_ZOOM    = 4;    // magnifier zoom multiplier
const MAG_OFFSET  = 24;   // gap between cursor and magnifier edge
const SNAP_RADIUS = 18;   // canvas-pixel snap search radius
...
function draw(canvas, img, calibPixels, series, selected, hovered, showCalibMarkers, transform) {
  ...
  series.forEach(s => {
    s.points.forEach((pt, idx) => {
      ...
      ctx.strokeStyle = '#000000bb'; ctx.lineWidth = 0.8; ctx.stroke();
      ctx.fillStyle = '#fff';
...
function drawMagnifier(magCanvas, mainCanvas, cx, cy) {
  ...
  const srcH = half / MAG_ZOOM;

  ctx.clearRect(0, 0, MAG_SIZE, MAG_SIZE);
  ctx.save();
  ctx.beginPath(); ctx.arc(half, half, half, 0, Math.PI * 2); ctx.clip();
  ctx.fillStyle = '#0f172a'; ctx.fillRect(0, 0, MAG_SIZE, MAG_SIZE);
  ctx.drawImage(mainCanvas, cx - srcH, cy - srcH, srcH * 2, srcH * 2, 0, 0, MAG_SIZE, MAG_SIZE);
  ctx.restore();
  ...
  ctx.beginPath(); ctx.arc(half, half, 2, 0, Math.PI * 2);
...
export default function PlotCanvas({
  ...
  const snapRingRef  = useRef(null);
  const transformRef = useRef({ scale: 1, offsetX: 0, offsetY: 0 });
  const dragRef      = useRef(null);
  const mouseRef     = useRef({ cx: 0, cy: 0, inside: false });
  const snapRef      = useRef(null); // { px, py } image coords of current snap target
  ...
  function computeSnap(cx, cy) {
    ...
    if (snapped) {
      ...
      if (snapRingRef.current) {
        snapRingRef.current.style.display = 'block';
        snapRingRef.current.style.left    = `${scx}px`;
        snapRingRef.current.style.top     = `${scy}px`;
      }
      return { cx: scx, cy: scy };
    }
    snapRef.current = null;
    if (snapRingRef.current) snapRingRef.current.style.display = 'none';
    return { cx, cy };
  ...
  function handleMouseDown(e) {
    const { cx: rawCx, cy: rawCy } = getXY(e);

    if (mode === 'calibrate') {
      const { px, py } = canvasToImg(rawCx, rawCy, transformRef.current);
      onCanvasClick?.(px, py);
      return;
    }

    // collect mode — honour snap if active
    const hit = hitTest(rawCx, rawCy, series, transformRef.current);
    if (hit) {
      dragRef.current = hit;
      onPointMouseDown?.(hit);
    } else {
      if (snapRef.current) {
        onCanvasClick?.(snapRef.current.px, snapRef.current.py);
      } else {
        const { px, py } = canvasToImg(rawCx, rawCy, transformRef.current);
        onCanvasClick?.(px, py);
      }
  ...
  function handleMouseMove(e) {
    const { cx: rawCx, cy: rawCy } = getXY(e);
    mouseRef.current = { cx: rawCx, cy: rawCy, inside: true };

    // Compute snap (returns snapped canvas coords or original)
    const { cx, cy } = computeSnap(rawCx, rawCy);

    // Magnifier centres on snap point (or cursor if no snap)
    refreshMagnifier(cx, cy);
    ...
    if (dragRef.current && e.buttons === 1) {
      const { px, py } = canvasToImg(rawCx, rawCy, t);
      onDrag?.(dragRef.current, px, py);
      return;
    }

    const hit = hitTest(rawCx, rawCy, series, t);
    onHoverChange?.(hit);
  ...
  return (
    <div ref={containerRef} className="relative h-full w-full overflow-hidden rounded-2xl border border-white/10 bg-slate-900">
      ...
      {/* Snap ring — cyan circle at snapped edge pixel */}
      <div
        ref={snapRingRef}
        style={{
          display:       'none',
      ...
      >
        <canvas ref={magnifierRef} width={MAG_SIZE} height={MAG_SIZE} style={{ borderRadius: '50%' }} />
      </div>
    </div>
  );
}
